...
import { Plus, Pencil, Trash2, Eye, EyeOff, Globe, Shield, TrendingUp, Award } from "lucide-react";
...
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = { Globe, Shield, TrendingUp, Award };
...
function BeneficiosAdmin() {
  ...
  const [editingItem, setEditingItem] = useState<BenefitForm | null>(null);
  const [form, setForm] = useState<BenefitForm>(emptyForm);

  const { data: benefits, isLoading } = useQuery(trpc.benefits.list.queryOptions());

  const createMutation = useMutation(
    trpc.benefits.create.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries(trpc.benefits.list.queryOptions());
        queryClient.invalidateQueries(trpc.content.getBenefits.queryOptions());
  ...
  const updateMutation = useMutation(
    trpc.benefits.update.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries(trpc.benefits.list.queryOptions());
        queryClient.invalidateQueries(trpc.content.getBenefits.queryOptions());
        queryClient.invalidateQueries(trpc.content.getHomepageData.queryOptions());
  ...
  const deleteMutation = useMutation(
    trpc.benefits.delete.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries(trpc.benefits.list.queryOptions());
        queryClient.invalidateQueries(trpc.content.getBenefits.queryOptions());
        queryClient.invalidateQueries(trpc.content.getHomepageData.queryOptions());
  ...
  const openModal = (item?: BenefitForm) => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600">Sección "¿Por qué elegirnos?"</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {isLoading ? (
          ...
          <div className="col-span-full p-8 text-center text-slate-500">
            No hay beneficios. <button onClick={() => openModal()} className="text-primary-600 font-medium">Crear uno</button>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="p-6 border-b border-slate-200">
              <h2 className="text-xl font-bold">{editingItem ? "Editar" : "Nuevo"} Beneficio</h2>
            </div>
